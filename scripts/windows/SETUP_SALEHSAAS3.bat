...
REM WARNING:
REM This installer represents a legacy stack profile and is not a runtime authority.
REM Always validate active services and ports against docker-compose.yml first.
...
echo  📥 تحميل نموذج التضمين التشغيلي nomic-embed-text:latest ^(للـ RAG^)...
docker exec salehsaas_ollama ollama pull nomic-embed-text:latest
if %errorLevel% neq 0 (
    echo  ⚠️  لم يتم تحميل نموذج التضمين التشغيلي. يمكنك تحميله لاحقاً.
) else (
    echo  ✅ تم تحميل نموذج التضمين التشغيلي
)

if /I "%ALLOW_LEGACY_QWEN3%"=="true" (
    echo  📥 Legacy opt-in: تحميل qwen3-embedding:0.6b ...
    docker exec salehsaas_ollama ollama pull qwen3-embedding:0.6b
    if %errorLevel% neq 0 (
        echo  ⚠️  فشل تحميل qwen3-embedding:0.6b.
    ) else (
        echo  ✅ تم تحميل qwen3-embedding:0.6b ^(legacy optional^)
    )
) else (
    echo  ⏭️  تم تجاوز qwen3-embedding legacy. للتفعيل: ALLOW_LEGACY_QWEN3=true
)
...
echo  ║                                                                  ║
echo  ║  🖥️  الطرفية التفاعلية ^(Open Terminal^):                         ║
echo  ║      http://localhost:8000                                       ║
echo  ║                                                                  ║
echo  ║  ⚙️  أتمتة سير العمل ^(n8n^):                                    ║
echo  ║      http://localhost:5678                                       ║
echo  ║                                                                  ║
echo  ║  🔌  Data Pipeline API:                                           ║
...
echo  ║                                                                  ║
echo  ╠══════════════════════════════════════════════════════════════════╣
echo  ║  ⚠️  خدمات Legacy الاختيارية غير مفعلة افتراضياً:                ║
echo  ║      AnythingLLM, Code Server, Grafana                           ║
echo  ║  المرجع الأعلى دائماً: docker-compose.yml                        ║
echo  ╠══════════════════════════════════════════════════════════════════╣
echo  ║                                                                  ║
