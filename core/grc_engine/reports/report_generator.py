...
class GRCReportGenerator:
    ...
    def generate_markdown_report(self, results, output_path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("# 🛡️ تقرير امتثال الحوكمة والمخاطر (GRC) - SaleHSaaS 4.0\n\n")
            ...
            for framework, result in results.items():
                ...
                summary_table += f"| {result.get('framework', framework)} | {status_ar} | {findings_count} |\n"

            f.write(summary_table)
            f.write("\n---\n\n## التفاصيل\n\n")

            for framework, result in results.items():
                f.write(f"### {result.get('framework', framework)}\n\n")
                ...
                for finding in result['findings']:
                    ...
                    severity_ar = severity_map.get(finding.get('severity'), finding.get('severity', 'Unknown'))
                    findings_table += f"| {finding.get('control_id', 'N/A')} | {finding.get('description', 'N/A')} | {severity_ar} |\n"
